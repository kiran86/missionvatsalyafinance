...
class DbFunction{
	...
	function generate_csv($fy_qtr) {
		$db = Database::getInstance();
		$mysqli = $db->getConnection();
		$query = "SELECT cci.id, cci.district, cci.cci_name, cci.cci_run_by, COUNT(cci.cci_unit_no) AS 'No. of Units', cci.category, cci.cci_gender
					...
					GROUP BY cci.cci_name, cci.category, cci.cci_gender
		...
		fputcsv($output, array('id', 'district', 'cci_name', 'cci_run_by', 'cci_unit_no', 'category', 'cci_gender', 'fy_id', 'children_days', 'cwsn_children_days' ,'dist_recommendation'));
